...
import { useAuthStore, useToastStore, useModelStore } from '../../store';
...
export const Icon = ({ name, size = 16, ...props }) => {
  const paths = {
    ...
    building: <><rect x="4" y="2" width="16" height="20" rx="1"/><line x1="9" y1="22" x2="9" y2="12"/><line x1="15" y1="22" x2="15" y2="12"/><rect x="9" y="12" width="6" height="4"/><line x1="9" y1="7" x2="9.01" y2="7"/><line x1="15" y1="7" x2="15.01" y2="7"/></>,
...
export function Sidebar() {
  ...
  const projectId = storeProjectId || (paramProjectId ? parseInt(paramProjectId) : null);
  const navigate = useNavigate();

  const handleLogout = () => { logout(); useModelStore.getState().reset(); navigate('/login'); };
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        <div style={{ fontSize: '0.75rem', color: 'var(--sidebar-muted)', marginBottom: '0.75rem' }}>
...
export function AppShell({ children, title, actions }) {
  // Read only the projectName slice — avoids re-rendering on every store change
...
// ── Read-only banner (kept for any future read-only pages) ────────────────
